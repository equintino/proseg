...
final class Utils {
     ...
     */
    public static function getrncByGetId() {
        $id = null;
        try {
            $id = self::getUrlParam('id');
        } catch (Exception $ex) {
            throw new NotFoundException('No RNC identifier provided.');
        }
        if (!is_numeric($id)) {
            throw new NotFoundException('Invalid RNC identifier provided.');
        }
        $dao = new rncDao();
        $rnc = $dao->findById($id);
        if ($rnc === null) {
            throw new NotFoundException('Unknown RNC identifier provided.');
        }
        return $rnc;
    }
